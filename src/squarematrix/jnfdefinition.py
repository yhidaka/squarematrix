...
def rkchop(m):
        ...
        while rank > 0:
                ...
                if maxmp<tol:
                        ...
                        return array(rkm)
                mp=mp/maxmp    #when mp is too small, we need renormalize mp#removed 11/19/2019
...
def nullblocksvd(s):#see JordanNormalFormAndTriangularMatrix.nb in /Users/lihuayu/Desktop/nonlineardynamics/nonlineaDyn/Relation to driving terms/Jordan Form Reformulation
...
def triangularsubblocks(s1):
    ...
    ml = np.append(ml,ln) #When the last block of nullspace has dimension greater than 1, this step is critical so that QR decomposition for the last block is treated properly.
...
def semiJ(J):
    ...
    ln = len(J)
    if display=="semiJ": print("ln=",ln)
    if display=="semiJ": sv('junk',J)
    if display=="semiJ": prm(J,10,10)
    ch = findchains(J)
    lst=list(map(len,ch))
...
def semileftjordanbasis(mn):
        ...
        if np.amax(abs(tm))<tol:return xi, 0*tm #if so then J=tm=0, u=xi , i.e., all xi are proper eigenvectors
...
def correlationatorder(u,chainposition,powerindex,index,lowestpower,ypowerorder):
        ...
        for i in range(len(index)):
                ...
                for j in range(len(index)):
                        ...
                        mcc[i,j]=orderdot.orderdot(ht(u[index[i]]),u[index[j]], order,powerindex,ypowerorder)
...
def correlationatordernew(u,chainposition,powerindex,ypowerorder,u0idx):
        ...
        for i in range(1,len(u)):
                ...
                for j in range(1,len(u)):
                        ...
                        mcc[i-1,j-1]=orderdot.orderdot(ht(u[i]),u[j], order,powerindex,ypowerorder)
...
def minimizeu0byorder(u,J,chainposition, powerindex,u0idx, ypowerorder):
        ...
        maxchainlenposition, maxchainlen, chain, chainposition=findchainposition(J)
        lowestpower=[ findordernew(i,chainposition,u,powerindex, ypowerorder,u0idx) for i in range(len(u))]
        index1=[ i for i in range(len(u)) if lowestpower[i]==1]
        index=[ i for i in range(len(u)) if lowestpower[i]!=1]
        #Construct the matrix equation for the transformation coefficients abeta
        #if ypowerorder==0, then use orderdot with x power counted only, else y powers are also take into account
        mc=correlationatorder(u,chainposition, powerindex,index, lowestpower,ypowerorder) #the matrix <e_i|e_j>_{power of i}
        ene0=np.empty(len(index))*(1.+0.j) #column <e_i|e_0>
        for i in range(len(index)):
                order=lowestpower[index[i]] #For each i select the power order for the minimization
                #ene0[i]=orderdotnew(ht(u[index[i]]),u[0], order, powerindex, ypowerorder)
                ene0[i]=orderdot.orderdot(ht(u[index[i]]),u[0], order, powerindex, ypowerorder)

        abeta=np.linalg.solve(mc,-ene0) #solve mc.X=-ene1
        for i in index1:
                abeta=np.insert(abeta,i,0) #this makes it easy to use indexes to identify which ratio is used later
        ...
        #construct the cross block upper triangular matrix for substracting vector in other chains from the main vector in the main chain
        for k in range(0, len(chainposition)): #for all chain except the main chain construct the upper-triangular part of the transformation matrix
                link=chainposition[k]
...
def UMexpJ(M, mu,powerindex,u0idx, ypowerorder):
        ...
        u2,t2=minimizeu0byorder(u1,JlnJM,chainposition, powerindex,u0idx, ypowerorder) #Hence u2=t2.u1 is still the left basis of ln(exp(-j*mu)M):   u2.ln(exp(-j*mu)M)=JlnJM.u2, and the high order terms in u2[0] are minimized
...
def UMexpJnew(M, mu,powerindex,nres,u0idx,ypowerorder):
        ...
        if np.amax(abs(Jn))<tol: return un,Jn,np.identity(len(un))*(1+0j),un
...
def scalingnew(Ms1,powerindex,scalemf):
        ...
        ms=d3(As2,Ms1,Asm2) #Notice!! ms does not take the first scaling into account, so ms cannot operate on Z column made from zbar, not even zbars which is scaled twice from zbar to zbar/scalefm to zbar/scalex
...
#Hence u0idx=sequencenumber[0,0,1,0] here is the position of the dominating lowest order term in u, when we #consider muy, we use the term [0,0,1,0]
        ...
        tm1,g=righteigensubspace(mn) #the calculation of ubar is based on derivation in "Resonance Blocks.one" in the folder /nonlineardyn/Relation to driving term/Jordan Form Reformulation
        A=np.dot(u,g)
        Am=np.linalg.inv(A)
        ubar=np.dot(g,Am) #ubar here is the E=g.(f.g)^-1=G(UG)^-1 in  "Resonance-Blocks-Calculation-of-reciprocal-basis.pdf"
        ...
        if np.amax(abs(J))<tol:
                tmp=map(np.argmax, abs(u))#When J=0, all rows in u are eigenvectors, we need to order them according to u with lower power first
                tmp1=[ sum(powerindex[i]) for i in tmp] #tmp1 has the index of the maximum term in every row of u
                tmp2=np.argsort(tmp1)#tmp2 sort u according to the lowest power of the maximum term arranged first
                u=u[tmp2]
