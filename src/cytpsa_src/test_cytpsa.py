...
def test_TPS_addition():
    ...
    for i, _exp in enumerate(_t.T['exponents']):
        np.testing.assert_almost_equal(_t.get_Taylor_coeff(_exp),
                                       all_taylor_coeffs[i], decimal=16)
    ...
    f1 = lambda x, px, y, py: 1.5 * x**2 - 2.4 * px + 0.8 * x * py - 14.0 * py**3 + 2 * x**4
    f2 = lambda x, px, y, py: 3.3 * x + 2.3 * px - 1.1 * px**2 + 8.9 * x**2 * px * py - 9.1 * x * px * y * py
    ...
    for _s, _t in [(s1, t1), (s2, t2), (s1 + s2, t1 + t2), (s1 * s2, t1 * t2),
                   (s3, t3)]:
        a = Poly(_s, x, px, y, py)
        s_monoms = a.monoms()
        for exp in _t.T['exponents']:
            ...
            np.testing.assert_almost_equal(
                _t.get_Taylor_coeff(exp), s_c, decimal=12)


    # Test differentiation
    for _s, _t in [(s1, t1), (s2, t2)]: # You cannot test "s1 * s2" as its degree is 8 > norder
        s_diff = diff(_s, x)
        t_diff = _t.diff('x')
    ...
    f1 = lambda x, px, y, py, cdt, delta: 1.5 * x**2 - 2.4 * px + 0.8 * x * py - 14.0 * py**3 + 2 * x**4 + 1.25 * cdt**3 * delta - 4.3 * delta**3
    f2 = lambda x, px, y, py, cdt, delta: 3.3 * x + 2.3 * px - 1.1 * px**2 + 8.9 * x**2 * px * py - 9.1 * x * px * y * py - 14.9 * cdt * x * px * y + 9.97 * delta*2 * y**2
...
def test_TPS_division(complex_coeffs):
    ...
    for k in sorted(list(s_ans)):
        ...
        for exp in _t.T['exponents']:
            ...
            try:
                ...
                print('rel. diff: {:.4e}'.format(
                    np.abs(_t.get_Taylor_coeff(exp) / complex(s_c.evalf())) - 1.0))
                rel_tol = 1e-12
                assert np.abs(_t.get_Taylor_coeff(exp) /
                              complex(s_c.evalf()) - 1.0) < rel_tol
    ...
    for _s, _t in [
        (s1, t1), (s2, t2), (s1 / 1.5, t1 / 1.5), (s1 / s2, t1 / t2),
        (s1 / 1.5, div_1_in_place), (s1 / s2, div_2_in_place)]:
        a = Poly(_s, x, p)
        s_monoms = a.monoms()
        for exp in _t.T['exponents']:
            if tuple(exp) not in s_monoms:
...
def test_TPS_exponential(complex_coeffs):
    ...
    if not complex_coeffs:
        ...
        t_ans['s3'] = -3.0 * t_p * cytpsa.exp(3 * t_x + 1) + 9.0 * (t_p**2) - 4.5 * ((t_x**4) * t_p)
        s_ans['s3'] = (-3.0 * p * sym_exp(3 * x + 1) + 9.0 * (p**2) - 4.5 * (x**4) * p).series(
            x, n=norder+1)
    else:
        t_ans['s1'] = cytpsa.exp(t_x)
        s_ans['s1'] = (sym_exp(x)).series(n=norder+1)
        ...
        t_ans['s3'] = -3.0 * t_p * cytpsa.exp(3j * t_x + 1) + 9.0j * (t_p**2) - 4.5 * ((t_x**4) * t_p)
        s_ans['s3'] = (-3.0 * p * sym_exp(3j * x + 1) + 9.0j * (p**2) - 4.5 * (x**4) * p).series(
            x, n=norder+1)
...
def test_TPS_log(complex_coeffs):
    ...
    if not complex_coeffs:
        ...
        t_ans['s3'] = -3.0 * t_p * cytpsa.log(3 * t_x + 1) + 9.0 * (t_p**2) - 4.5 * ((t_x**4) * t_p)
        s_ans['s3'] = (-3.0 * p * sym_log(3 * x + 1) + 9.0 * (p**2) - 4.5 * (x**4) * p).series(
            x, n=norder+1)
    else:
        ...
        s_ans['s2'] = (1.5 * sym_log((1 + x * 3j) / (1 - x)) - 5.0j).series(n=norder+1)

        t_ans['s3'] = -3.0j * t_p * cytpsa.log(3j * t_x + 1) + 9.0 * (t_p**2) - 4.5j * ((t_x**4) * t_p)
        s_ans['s3'] = (-3.0j * p * sym_log(3j * x + 1) + 9.0 * (p**2) - 4.5j * (x**4) * p).series(
            x, n=norder+1)
...
def test_TPS_sqrt(complex_coeffs):
    ...
    if not complex_coeffs:
        ...
        s_ans['s2'] = (1.5 * sym_sqrt((3 + x * 2) / (1 - 4 * x**3)) - 5.0).series(n=norder+1)

        t_ans['s3'] = -3.0 * t_p * cytpsa.sqrt(3 * t_x + 1) + 9.0 * (t_p**2) - 4.5 * ((t_x**4) * t_p)
        s_ans['s3'] = (-3.0 * p * sym_sqrt(3 * x + 1) + 9.0 * (p**2) - 4.5 * (x**4) * p).series(
            x, n=norder+1)
    else:
        ...
        s_ans['s2'] = (1.5 * sym_sqrt((3 + x * 2j) / (1 - 4j * x**3)) - 5.0j).series(n=norder+1)

        t_ans['s3'] = -3.0 * t_p * cytpsa.sqrt(3j * t_x + 1) + 9.0 * (t_p**2) - 4.5j * ((t_x**4) * t_p)
        s_ans['s3'] = (-3.0 * p * sym_sqrt(3j * x + 1) + 9.0 * (p**2) - 4.5j * (x**4) * p).series(
            x, n=norder+1)
...
def _test_TPS_trig(trig_name, complex_coeffs):
    ...
    if not complex_coeffs:
        ...
        t_ans['s3'] = -3.0 * t_p * cytpsa_trig(3 * t_x + 1) + 9.0 * (t_p**2) - 4.5 * ((t_x**4) * t_p)
        s_ans['s3'] = (-3.0 * p * sym_trig(3 * x + 1) + 9.0 * (p**2) - 4.5 * (x**4) * p).series(
            x, n=norder+1)
    else:
        ...
        s_ans['s2'] = (1.5 * sym_trig((1 + x * 3j) / (1j - x)) - 5.0j).series(n=norder+1)

        t_ans['s3'] = -3.0 * t_p * cytpsa_trig(3j * t_x + 1) + 9.0 * (t_p**2) - 4.5j * ((t_x**4) * t_p)
        s_ans['s3'] = (-3.0 * p * sym_trig(3j * x + 1) + 9.0 * (p**2) - 4.5j * (x**4) * p).series(
            x, n=norder+1)
