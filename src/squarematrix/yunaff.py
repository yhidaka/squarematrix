...
def interpolated_fft(
    xi, rng=[0.0, 1], ith=1, hann=True, verbose=False, semilog=False, figsize=(10, 3)
):
    """
    ...
    dasp = [
        [j, j * 1.0 / T, asp]
        for j, asp in enumerate(xpw)
        if 0 < j < T - 1
        and rngi[0] <= j <= rngi[1]
        and xpw[j - 1] < xpw[j] > xpw[j + 1]
    ]
    if xpw[0] > xpw[1]:
        dasp.insert(0, [0, 0.0, xpw[0]])
    if xpw[len(xpw) - 1] > xpw[len(xpw) - 2]:
        dasp.append([len(xpw), len(xpw) * 1.0 / T, xpw[len(xpw) - 1]])
    dasp = np.array(
        dasp
    ).transpose()  # select all points with two points next to it having amplitudes smaller than the middle point
    idxdasp = np.argsort(dasp[2])
    idxpeaks = dasp[0][
        idxdasp
    ]  # the positions in the fft spectrum where middle point is larger than its two sides, sorted according to the differences from the sides
    tunesorted = idxpeaks / T  # The tune of the sorted peaks
    # print "tunesorted[-5:]=\n",tunesorted[-5:]
    maxi = int(
        idxpeaks[-1]
    )  # the position of the point with maximum of smaller difference with its neighbor
    lk = xpw[maxi]
    lkm = xpw[maxi - 1]
    lkp = xpw[maxi + 1]
    A = (lkp - lkm) / 2.0 / (2.0 * lk - lkp - lkm)
...
def naffref(
    f, fref=[0.058, 0.065, 0.051], df=2e-3
):  # naffref use specified tunes fref to find peaks tune within df of fref, eg. 0.058-df<nu<0.058+df
    """
    ...
    scale = 0.1
    for i in range(ni):
        nu0, tunesorted = interpolated_fft(
            fc, rng=[fref[i] - df, fref[i] + df]
        )  # find the preliminary tune approximation using 3 points in the spectrum
        ...
        nu1 = yufminscaled(nu0, fc.copy(), hw, nu0, scale)[
            0
        ]  # find the tune by maximize the projection weighted by hw,
        nus[i] = nu1
        ...
        if i == 0:
            ...
            d2[0] = prodhw(
                d[0], d[0], hw
            )  # vector product of d[0].d[0].hw, i.e. with weight hw (called window by hw)
            ...
            for k in range(i):
                djui[k][i] = prodhw(
                    d[k], u[i], hw
                )  # vector product of d[k].u[i].hw, i.e. with weight hw (called window by hw)
                # d[i]=d[i]-  djui[j][i]  *d[j]  /d2[j]
            d[i] = u[i] - sum(
                djui[k][i] * d[k] / d2[k] for k in range(0, i)
            )  # Gramm-Schmidt orthogonalization step to produce the new basis vector d_i
            expdiuj[i] = expdiuj[i] - sum(
                djui[k][i] * expdiuj[k] / d2[k] for k in range(i)
            )
            d2[i] = prodhw(d[i], d[i], hw)  # d2[i] is the norm of d_i with weight hw
        fcdj[i] = prodhw(
            d[i], fc, hw
        )  # fcdj is projection with weight hw on the residual fc of d_i which is not normalized by sqrt(d2_i) yet.
        # print "i=",i, "fcdj[i]=", fcdj[i], " fcdj[i]/d2[i]=",fcdj[i]/d2[i], "d2[i]=",d2[i]
        fc = (
            fc - d[i] * fcdj[i] / d2[i]
        )  # d[i]/dqrt(d2[i]) is the d_i normalized according to weight hw
