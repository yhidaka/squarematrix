...
def strfind(string, pattern):
    ...
    return [s.start() for s in re.finditer(pattern, string)]
...
def query(sdds_filepath, suppress_err_msg=False):
    ...
    p = Popen(["sddsquery", sdds_filepath], stdout=PIPE, stderr=PIPE, encoding="utf-8")
        ...
        for index, s in enumerate(param_str_list):
            ss = re.search(
                r"([\w\./]+) +({0:s}) +({1:s}) +({2:s}) +(.+)".format(
                    unit_pattern, symbol_pattern, type_pattern
                ),
                s,
            ).groups()
            param_dict[ss[0]] = {
...
def printout(
                ...
                for param_name, q_d in param_info_dict.items():
                    ...
                    if False:  # old version before dealing with SDDS "pages"
                        ...
                        try:
                            ...
                            vals = [
                                v.replace(next_param_name, "").strip() for v in vals
                            ]
                        except IndexError:
                            pass
    ...
    if column_name_list == []:  # or (column_name_list is None):
        ...
        if not use_comma_delimiter:
            ...
            if column_start_ind != []:
                column_start_ind = column_start_ind[0] + len(
                    column_title_divider_pattern
                )
                ...
                for r in rows_str:
                    ...
                    for j, st in enumerate(str_list):
                        column_dict[column_name_list[j + col_ind_offset]][
                            row_counter
                        ] = st
                ...
                for col_name in column_name_list:
                    column_dict[col_name] = column_dict[col_name][
                        :row_counter
                    ]  # Make sure to
...
def dicts2sdds(
    ...
    plaindata2sdds(
        plaindata_txt_filepath,
        sdds_output_filepath,
        outputMode=outputMode,
        param_name_list=param_name_list,
        param_type_list=param_type_list,
        param_unit_list=param_unit_list,
        param_descr_list=param_descr_list,
        param_symbol_list=param_symbol_list,
        param_count_list=param_count_list,
        column_name_list=column_name_list,
        column_type_list=column_type_list,
        column_unit_list=column_unit_list,
        column_descr_list=column_descr_list,
        column_symbol_list=column_symbol_list,
